...
def is_sub_expire(user: tables.User) -> bool:
    ...
    print(user.subscription[0].expire_date.date())
    print(datetime.datetime.now(tz=timezone('Europe/Moscow')).date())
...
async def kick_user_from_channel(user_tg_id: int, bot: aiogram.Bot):
    ...
    print(f"УДАЛЕНИЕ ПОЛЬЗОВАТЕЛЕЙ {user_tg_id}")
...
async def notify_user_about_expiration(user_tg_id: int, bot: aiogram.Bot):
    ...
    await bot.send_message(user_tg_id, "Ваша подписка закончилась, вы можете приобрести подписку в главном меню /start")
